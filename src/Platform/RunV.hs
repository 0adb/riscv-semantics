...
runProgram :: Maybe Int64 -> Minimal64V -> IO (Int64, Minimal64V)
runProgram maybeToHostAddress c = do
  putStrLn "Start"
  runStateT (stepHelper RV64IMV maybeToHostAddress (do 
...
runFile f = do
  ...
  let c = Minimal64V { registers = S.empty,
                       vregisters = (take 32 $ (repeat (take 8 (repeat 0b0)))),
                      fpregisters = (take 32 $ repeat 0),
                      csrs = (resetCSRFileV 64),
                      pc = 0x80000000,
                      nextPC = 0,
                      privMode = Machine,
