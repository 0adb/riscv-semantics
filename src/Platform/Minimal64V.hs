...
module Platform.Minimal64V where
import Spec.Machine
import Spec.Decode
...
data Minimal64V = Minimal64V { registers :: S.Map Register Int64,
...
instance RiscvMachine MState Int64 where
  ...
  getVRegister reg = state $ \comp -> ((vregisters comp) !! ((fromIntegral :: VRegister -> Int) reg), comp)
  setVRegister reg val = state $ \comp -> ((), comp { vregisters = setIndex ((fromIntegral:: VRegister -> Int) reg) val (vregisters comp) })
  ...
  -- CSRs:
  getCSRField field = state $ \comp -> (if field /= Field.VLenB then getField field (csrs comp) else 8, comp)
